...
for img_file in image_files:
    ...
    museum_name = image_to_museum.get(img_file.name, "미상")
    image_meta = {
...
# 미술관 자동 추론 함수 (파일명 기반)
# ===========================
def infer_museum_from_filename(filename):
    if filename.startswith("MMCA"):
        return "국립현대미술관"
    elif filename.startswith("SEOMA"):
        return "서울시립미술관"
    else:
        return "미상"
...
for img_file in image_files:
    print(f"🔍 {img_file.name} 처리 중...")
    museum_name = infer_museum_from_filename(img_file.name)
    image_meta = {
        "full_image_id": img_file.name,
        "full_image_description": f"Placeholder description for {img_file.name}",
        "museum_name": museum_name,
